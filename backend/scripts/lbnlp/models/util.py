...
def check_versions(reqs):
    ...
    for req in reqs:
        if "==" in req:
            ...
            warnings.warn(
                f"Requirement {req} must be installed for this model package to function. Please ensure this package from git is installed from source. {suffix}"
            )
        else:
            raise ModelRequirementError(
    ...
    def decorator(loader_func):
        def wrapper(model_name, ignore_requirements=False):
            ...
            if model_name not in pkg.model_names:
                raise ValueError(
                    f"Model {model_name} in {pkg.modelpkg_name} not found. Choose from {pkg.model_names}"
                )
