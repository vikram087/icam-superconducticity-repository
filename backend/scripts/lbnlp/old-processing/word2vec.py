...
class DataPreparation:
    ...
    def to_word2vec_corpus(
        self,
        filename="abstracts",
        limit=None,
        newlines=False,
        line_per_abstract=True,
        doi=None,
        only_relevant=False,
        exclude_punct=False,
        year_max=None,
        ...
        abstracts = self._get_abstracts(
            sample=sample, col=self.TOK_ABSTRACT_COL, doi=doi, year_max=year_max
        )
    ...
    def tokenize(text):
        """
        Returns a list of tokens using spaCy tokenizer.
        ...
        # Iterate over sentences and then tokens in each sentence
        for sent in doc.sents:
            toks.append([token.text for token in sent])
    ...
    def tokenize_abstracts(self, limit=None, override=False, sample=False):
        ...
        existing_dois = [
            abstr[self.DOI_FIELD]
            for abstr in self._get_abstracts(col=self.TOK_ABSTRACT_COL)
        ]
        ...
        def insert_abstract(a):
            if override or (not override and a[self.DOI_FIELD] not in existing_dois):
                ...
                try:
                    ...
                    print(
                        "Exception type: %s, doi: %s"
                        % (type(e).__name__, a[self.DOI_FIELD])
                    )
                    abs_tokens = {
                        self.DOI_FIELD: a[self.DOI_FIELD],
                        self.TTL_FILED: None,
                ...
                if override:
                    ...
                    getattr(self._db, self.TOK_ABSTRACT_COL).replace_one(
                        {"doi": a[self.DOI_FIELD]}, abs_tokens, upsert=True
                    )
                else:
                    try:
    ...
    def _get_abstracts(
        self, limit=None, col=None, doi=None, year_max=None, sample=True
    ):
        """
        ...
        if limit is not None:
            if sample:
                pipeline = [{"$match": conditions}, {"$sample": {"size": limit}}]
            ...
            if sample:
                ...
                pipeline = [{"$match": conditions}, {"$sample": {"size": size}}]
                abstracts = getattr(self._db, col).aggregate(
                    pipeline, allowDiskUse=True
                )
            else:
    ...
    def process_sentence(
        self, s, exclude_punct=False, convert_num=True, normalize_mats=True
    ):
        """
            ...
            if exclude_punct and tok in self.PUNCT:
                ...
                if elem_with_valence is not None:
                    ...
                    self.mat_list.append(
                        (matmention, formula)
                    )  # exclude the valence state from name
                    ...
                    (
                        len(tok) == 1
                        or (len(tok) > 1 and tok[0].isupper() and tok[1:].islower())
                    )
                    and tok not in self.ELEMENTS
                    and tok not in self.UNITS
                    ...
                    if nr_unit is None or nr_unit.group(2) not in self.UNITS:
                        tok = deaccent(
                            tok
                        )  # matches the pattern but not in the list of units
                    else:
                        # splitting the unit from number
                        (
                            st.append("<nUm>")
                            if convert_num
                            else st.append(nr_unit.group(1))
                        )
                        split_indices.append(i)
        ...
        if self.VALENCE_INFO.search(text) is not None:
            ...
            try:
                ...
                if len(composition.keys()) < 2 or any(
                    [
                        not self.simple_parser.is_element(key)
                ...
                if any(
                    [
                        not self.simple_parser.is_element(key)
                        for key in self.parser.parse_formula(text).keys()
                    ]
                ):
                    return False
                return True
    ...
    def get_norm_formula(self, text, max_denominator=1000):
        try:
            ...
            integer_formula = self.get_ordered_integer_formula(
