...
class MaterialParser:
    def __init__(self, pubchem_lookup=False, data_path=None):
        ...
        self.__filename = (
            os.path.dirname(os.path.realpath(__file__)) if not data_path else data_path
        )
    ...
    def __is_correct_composition(self, formula, chem_compos):
        if chem_compos == {}:
            return False
        if any(
            el not in formula + "M" + "Ln" or amt == ""
            for el, amt in chem_compos.items()
        ):
            return False
    ...
    def __get_mixture(self, material_name):
        ...
        for m in re.finditer(
            ...
            mixture[m.group(1)]["composition"] = self.get_structure_by_formula(
                m.group(1)
            )["composition"]
            mixture[m.group(2)]["fraction"] = "x"
            mixture[m.group(2)]["composition"] = self.get_structure_by_formula(
                m.group(2)
            )["composition"]
            ...
            for i in [1, 2]:
                if m.group(i)[0] == "(" and m.group(i)[-1] == ")":
                    ...
                    parts = [
                        s
                        for s in re.split("[-+]{1}([\d\.]*[A-Z][^-+]*)", line)
                        if s != "" and s != line
                    ]
                    for s in parts:
                        ...
                        if fraction == "":
                            fraction = "1"
                        mixture[name[1]]["fraction"] = (
                            "(" + fraction + ")*" + mixture[m.group(i)]["fraction"]
                        )
                        mixture[name[1]]["composition"] = self.get_structure_by_formula(
                            name[1]
                        )["composition"]
        ...
        if mixture == {}:
            parts = [
                s
                for s in re.split(
                    "[-+]{1}([\d\.]*[A-Z][^-+]*)", material_name.replace(" ", "")
                )
                if s != "" and s != material_name.replace(" ", "")
            ]
            for s in parts:
                name = re.findall("([\d\.]*)([\(A-Z].*)", s.strip(" -+"))[0]
                mixture[name[1]] = {}
    ...
    def get_chemical_structure(self, material_name):
        ...
        if material_name[0].islower():
            for m in re.finditer(
                "([a-z" + "".join(self.__greek_letters) + "]*)-(.*)", material_name
            ):
                phase = m.group(1)
        ...
        if chemical_structure["mixture"] != {}:
            chemical_structure["formula"] = "".join(
                chemical_structure["mixture"].keys()
            )
        else:
        ...
        if not self.__is_correct_composition(
            chemical_structure["formula"], chemical_structure["composition"]
        ):
            if (
                material_name.lower() in self.__chemical_names
                or material_name in self.__chemical_names
            ):
        ...
        if (
            not self.__is_correct_composition(
                chemical_structure["formula"], chemical_structure["composition"]
            )
            and self.__pubchem
        ):
            ...
            if len(pcp_compounds) > 0:
                try:
                    t_struct = self.get_structure_by_formula(
                        pcp_compounds[0].molecular_formula
                    )
                except:
                    t_struct = self.__empty_structure()
        ...
        for part in chemical_structure["mixture"].values():
            for var in re.findall(
                "[a-z" + "".join(self.__greek_letters) + "]", part["fraction"]
            ):
    ...
    def __get_values(self, string, mode, count=None, default_value=0.1, incr=None):
        ...
        if mode == "range" and len(string) != 0:
            ...
            if len(interval) > 0:
                ...
                if incr != None:
                    values = [
                        round(start + i * incr, 4)
                        for i in range(round((end - start) / incr))
                    ]
        ...
        if mode == "values" and len(string) != 0:
            values = [
                round(float(sympy.simplify(c)), 4)
                for c in re.findall("[0-9\./-]+", string[0].strip(" "))
            ]
    ...
    def get_stoichiometric_values(self, var, sentence):
        ...
        if len(values) == 0:
            values = self.__get_values(
                re.findall(var + "\s*=\s*([0-9\.\,/and\s-]+)[\s\)\]\,]", sentence),
                mode="values",
            )
            mode = "values"
        # equal to range
        if len(values) == 0:
            values = self.__get_values(
                re.findall(var + "\s*=\s*([0-9-–\.\s]+)[\s\)\]\,m\%]", sentence),
                mode="range",
                count=10,
            )
            mode = "range"
        ...
        if len(values) == 0:
            values = self.__get_values(
                re.findall(
                    var + "[a-z\s]*from\s([0-9\./]+)\sto\s([0-9\./]+)", sentence
                ),
                mode="range",
                count=10,
            )
    ...
    def get_elements_values(self, var, sentence):
        ...
        if len(values) > 0:
            values = [
                c
                for c in re.split("[,\s]", values[0])
                if c in self.__list_of_elements_1 + self.__list_of_elements_2
            ]
    ...
    def __is_abbreviation(self, word):
        if (
            all(c.isupper() for c in re.sub("[0-9x\-\(\)\.]", "", word))
    ...
    def build_abbreviations_dict(self, materials_list, paragraphs):
        ...
        abbreviations_dict = {
            t: "" for t in materials_list if self.__is_abbreviation(t.replace(" ", ""))
        }
        ...
        for abbr, name in abbreviations_dict.items():
            ...
            if name == "":
                ...
                sents = " ".join(
                    [s.text for p in paragraphs for s in nlp(p).sents if abbr in s.text]
                ).split(abbr)
    ...
    def get_dopants(self, material_name):
        ...
        for r in ["coated", "activated", "modified", "stabilized", "doped"]:
            parts = [
                w
                for w in re.split("(.*)[-\s]{1}" + r + " (.*)", new_material_name)
                if w != ""
            ]
            if len(parts) > 1:
        ...
        if any(w in new_material_name for w in ["mol", "wt"]):
            parts = re.split(
                "[\-+]{0,1}[0-9\.]*[xyz]{0,1}\s*[mol]{3}\.{0,1}\%{0,1}",
                new_material_name,
            ) + re.split(
                "[\-+]{0,1}[0-9\.]*[xyz]{0,1}\s*[wt]{2}\.{0,1}\%{0,1}",
                new_material_name,
            )
            parts = sorted(
                list(set(parts) - set([new_material_name])),
                key=lambda x: len(x),
                reverse=True,
            )
    ...
    def get_compounds_dictionary(self):
        ...
        for line in open(
            os.path.join(self.__filename, "inorganic_compounds_dictionary"),
            encoding="utf-8",
        ):
        ...
        for line in open(
            os.path.join(self.__filename, "pub_chem_dictionary"), encoding="utf-8"
        ):
            name, formula = line.strip().split(" – ")
            name = name[0].lower() + name[1:]
            names_dict[name] = formula
