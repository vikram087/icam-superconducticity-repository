import json
from transformers import BertForQuestionAnswering, BertTokenizer, Trainer, TrainingArguments, BatchEncoding # type: ignore
...
with open(train_file_path) as f:
    train_dataset_dict: dict = json.load(f)
with open(validation_file_path) as f:
    validation_dataset_dict: dict = json.load(f)

# Prepare the datasets
train_dataset: Dataset = Dataset.from_dict(train_dataset_dict)
validation_dataset: Dataset = Dataset.from_dict(validation_dataset_dict)
...
def preprocess_function(examples: dict) -> BatchEncoding:
    ...
    answers: list[dict] = examples["answers"]
