...
def load(model_name, ignore_requirements=False, model_path=None):
...
class MatBERTNERModelWrapper:
    ...
    def __init__(self, model_name, basepath):
        self.model_file = os.path.abspath(
            os.path.join(basepath, "model_files/matbert-base-uncased")
        )

        sp_basedir = os.path.join(basepath, "state_paths")
        if model_name == "aunp2":
            state_path_dir = os.path.join(
                sp_basedir,
                "matbert_aunp2_paragraph_iobes_crf_10_lamb_5_1_012_1e-04_2e-03_1e-02_0e+00_exponential_256_80",
            )
        elif model_name == "aunp11":
            state_path_dir = os.path.join(
                sp_basedir,
                "matbert_aunp11_paragraph_iobes_crf_10_lamb_5_1_012_1e-04_2e-03_1e-02_0e+00_exponential_256_80",
            )
        elif model_name == "doping":
            state_path_dir = os.path.join(
                sp_basedir,
                "matbert_doping_paragraph_iobes_crf_10_lamb_5_1_012_1e-04_2e-03_1e-02_0e+00_exponential_256_80",
            )
        elif model_name == "solid_state":
            state_path_dir = os.path.join(
                sp_basedir,
                "matbert_solid_state_paragraph_iobes_crf_10_lamb_5_1_012_1e-04_2e-03_1e-02_0e+00_exponential_256_80",
            )
        else:
    ...
    def tag_docs(self, texts, device="cpu"):
        ...
        predictions = predict(
            ...
            state_path=self.state_path_file,
            scheme="IOBES",
            device=device,
            is_file=False,
        )
